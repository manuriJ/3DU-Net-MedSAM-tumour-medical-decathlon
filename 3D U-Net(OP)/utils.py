...
def plot_metrics(model_history, stop=100):
    ...
    fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(10, 4))
    axes.plot(range(stop), model_history['dice_coefficient'], label='Training', color='#FF533D')
    axes.plot(range(stop), model_history['val_dice_coefficient'], label='Validation', color='#03507E')
    axes.set_title('Dice Coefficient')
    axes.set_ylabel('DSC')
    axes.set_xlabel('Epoch')
    axes.legend(loc='upper right')
    fig.savefig('train_val_DSC.png', dpi=fig.dpi)