...
from model import (
    ...
    unet_3d_conv3dtranspose_without_batchnormalization
)
...
from classwise_evaluate import evaluate_segmentation
...
parser.add_argument('--model_architecture', default="conv3dtranspose_batchnormalization", type=str,
                    choices=["upsampling_batchnormalization",
...
parser.add_argument('--num_classes', type=int, required=True, default=4,
...
parser.add_argument('--num_channels', type=int, required=True, default=1,
...
parser.add_argument('--loss', type=str, default="log_dice", choices=["dice", "log_dice", "focal_loss"],
...
def build_unet_model_architecture(model_architecture, input_size, unet_resize_factor, unet_dropout_rate, num_classes,
    ...
    if model_architecture == "unet_3d_conv3dtranspose_without_batchnormalization":
        model = unet_3d_conv3dtranspose_without_batchnormalization(
            input_size=input_size,
            unet_resize_factor=unet_resize_factor,
            num_classes=num_classes,
            binary_model=binary_model
...
def main():
    ...
    train_mri_paths = [os.path.join(os.path.join(train_data_dir, "images"), x) for x in
                       os.listdir(os.path.join(train_data_dir, "images"))]
    train_mask_paths = [os.path.join(os.path.join(train_data_dir, "masks"), x) for x in
                        os.listdir(os.path.join(train_data_dir, "masks"))]

    val_mri_paths = [os.path.join(os.path.join(val_data_dir, "images"), x) for x in
                     os.listdir(os.path.join(val_data_dir, "images"))]
    val_mask_paths = [os.path.join(os.path.join(val_data_dir, "masks"), x) for x in
                      os.listdir(os.path.join(val_data_dir, "masks"))]
    ...
    overall_results = model.evaluate(val_datagenerator)
    results = {'DSC': overall_results[1],
               'DSC loss': overall_results[0],
               'Iou': overall_results[2]}
    classwise_results = evaluate_segmentation(model, val_datagenerator, num_classes, val_sample_weights)
    results['foreground_means'] = classwise_results

    # print(results)
    r1 = json.dumps(results, cls=NumpyEncoder)
    with open('results_nn.json', 'w') as f:
        json.dump(r1, f)
