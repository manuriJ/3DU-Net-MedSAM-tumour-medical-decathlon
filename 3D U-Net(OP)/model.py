...
def unet_3d_conv3dtranspose_without_batchnormalization(input_size=(240, 240, 160, 4), unet_resize_factor=2, num_classes=4, binary_model=False):
    """Constructs a U-Net 3D segmentation model without BatchNormalization layers, using Conv3DTranspose layers
    instead of UpSampling3D -> Conv3D layers.

    Args:
        input_size: (tuple) Keras model input shape (default: (240, 240, 160, 4)).
        unet_resize_factor: (int) Resize factor of the number of filters per Convolutional layer (default: 2).
        num_classes: (int) Number of classes in the training dataset (default: 4).
        binary_model: (boolean) If True, make the last layer have one filter with 'sigmoid' activation.
    """
    inputs = Input(shape=input_size)
    ...
    # Expansive path
    up6 = Conv3DTranspose(filters=512 // unet_resize_factor, kernel_size=(2, 2, 2), strides=(2, 2, 2), padding="same", kernel_initializer='he_normal')(conv5)
